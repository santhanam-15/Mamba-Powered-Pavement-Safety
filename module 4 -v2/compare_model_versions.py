...
def load_model(checkpoint_path: Path, device: torch.device) -> tuple[CMSegNet, float]:
    ...
    if "decoder.head.weight" in state_dict:
        out_channels = state_dict["decoder.head.weight"].shape[0]
    
    model = CMSegNet(img_size=IMG_SIZE, out_channels=out_channels).to(device)
...
def run_inference(
    ...
    # Handle multi-channel output by taking the mean across channels
    if probs.ndim == 3:  # Shape: (C, H, W)
        probs = probs.mean(axis=0)  # Average across channels -> (H, W)
...
def main():
    ...
    try:
        results.append(test_model_version("CMSegNet", args.checkpoint_v1, args.v1_path))
    except Exception as e:
        print(f"Error testing v1: {e}")
    
    try:
        results.append(test_model_version("CMSegNet + EH", args.checkpoint_v2, args.v1_path))
    except Exception as e:
        print(f"Error testing v2: {e}")
    
    try:
        results.append(test_model_version("CMSegNet + ShadowNet", args.checkpoint_v3, args.v1_path))
    except Exception as e:
    ...
    try:
        ...
        print(f"Permission denied creating directory: {args.output}")
        print("Attempting to use an alternative output directory...")
        args.output = Path("model_comparison_results_backup")
        args.output.mkdir(parents=True, exist_ok=True)
    except Exception as e:
    ...
    # Save results
    try:
        output_csv = args.output / "comparison_results.csv"
        results_df.to_csv(output_csv, index=False)
        print(f"\nResults saved to: {output_csv}")
    except PermissionError as e:
        print(f"Permission denied writing CSV: {e}")
        print("Trying to remove old file and retry...")
        try:
            output_csv = args.output / "comparison_results.csv"
            ...
            print(f"Failed to save after removing old file: {retry_error}")
