...
def test_model(
    ...
    with open(csv_path, "w", newline="", encoding="utf-8") as f:
        writer = csv.DictWriter(
            f,
            fieldnames=["sample_id", "TP", "TN", "FP", "FN", "Accuracy", "IoU", "F1"],
        )
        writer.writeheader()
    ...
    report_lines = [
        "=" * 80,
        "COMPREHENSIVE MODEL TEST REPORT (Per-Image Results)",
        "=" * 80,
    ...
    report_lines.extend([
        ...
        "-" * 80,
        f"{'Sample ID':<50} | {'IoU':>8} | {'Accuracy':>8} | {'F1':>8}",
        "-" * 80,
    ])
    
    for result in sorted(per_image_results, key=lambda x: x["IoU"], reverse=True):
        report_lines.append(
            f"{result['sample_id']:<50} | {result['IoU']:>8.6f} | "
            f"{result['Accuracy']:>8.6f} | {result['F1']:>8.6f}"
        )
    ...
    report_lines.extend([
        ...
        f"  - metrics.json (per-image metrics: TP, TN, FP, FN, Accuracy, IoU, F1)",
        "=" * 80,
    ])
